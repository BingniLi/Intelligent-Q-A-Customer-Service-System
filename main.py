...
async def do_get_answer_api(question: str, table_name: str = None):
    try:
        question,answer = do_get_answer(table_name, question, MODEL, MILVUS_CLI, MYSQL_CLI)
        return {'status': True,'question':question,'answer':answer }, 200
    except Exception as e:
        LOGGER.error(e)
        return {'status': False, 'msg': e}
'''       
@app.get('/qa/get_answer')
async def do_get_answer_api(question: str, table_name: str = None):
    try:
        question,answer,pic = do_get_answer(table_name, question, MODEL, MILVUS_CLI, MYSQL_CLI)
        return {'status': True,'question':question,'answer':answer}
    except Exception as e:
        LOGGER.error(e)
        return {'status': False, 'msg': e}
...
async def do_get_answer_api(question: str, table_name: str = None):
    try:
        question,answer,pic = do_get_answer(table_name, question, MODEL, MILVUS_CLI, MYSQL_CLI)
        if pic== 'None':
            return {'status': True,'pic': 'no pic in this answer'}, 200
        else:
            file_like = open(pic, mode = "rb")
...
if __name__ == '__main__':
    #uvicorn.run(app=app, host='42.193.20.32', port=8000)
