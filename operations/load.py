...
def extract_features(file_dir, model):
    try:
        ...
        return question_data, answer_data, pic_path, sentence_embeddings
...
def format_data(ids, question_data, answer_data, pic_path):
    data = []
    for i in range(len(ids)):
        value = (str(ids[i]), question_data[i], answer_data[i], pic_path[i])
        data.append(value)
    return data
...
def do_load(table_name, file_dir, model, milvus_client, mysql_cli):
    ...
    question_data, answer_data, pic_path, sentence_embeddings = extract_features(file_dir, model)
    ids = milvus_client.insert(table_name, sentence_embeddings)
    milvus_client.create_index(table_name)
    mysql_cli.create_mysql_table(table_name)
    mysql_cli.load_data_to_mysql(table_name, format_data(ids, question_data, answer_data, pic_path))
    return len(ids)
