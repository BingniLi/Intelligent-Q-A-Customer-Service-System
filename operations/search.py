import sys

sys.path.append("..")
from config import TOP_K, DEFAULT_TABLE
from logs import LOGGER


def do_search(table_name, question, model, milvus_client, mysql_cli):
    try:
        if not table_name:
            table_name = DEFAULT_TABLE
        feat = model.sentence_encode([question])
        results = milvus_client.search_vectors(table_name, feat,5)
        vids = [str(x.id) for x in results[0]]
        # print('--------------------', vids, '-----------------')
        questions = mysql_cli.search_by_milvus_ids(vids, table_name)
        distances = [x.distance for x in results[0]]
        return questions, distances
    except Exception as e:
        LOGGER.error(f" Error with search : {e}")
        sys.exit(1)


def do_get_answer_1(table_name, question, mysql_cli):
    try:
        if not table_name:
            table_name = DEFAULT_TABLE
        answer = mysql_cli.search_by_question(question, table_name)
        return answer
    except Exception as e:
        LOGGER.error(f" Error with search by question : {e}")
        sys.exit(1)
        
        
        
def do_get_answer(table_name, question, model, milvus_client, mysql_cli):
    try:
        if not table_name:
            table_name = DEFAULT_TABLE
        feat = model.sentence_encode([question])
        results = milvus_client.search_vectors(table_name, feat, TOP_K)
        vids = [str(x.id) for x in results[0]]
        answers = mysql_cli.search_by_milvus_id_for_answer(vids, table_name)
        questions = mysql_cli.search_by_milvus_ids(vids, table_name)
        pics = mysql_cli.search_by_milvus_id_for_pic(vids, table_name) #!
        answer = answers[0]
        question_find = questions[0]
        pic_path = pics[0] #!
        return question_find,answer,pic_path
    except Exception as e:
        LOGGER.error(f" Error with search by question : {e}")
        sys.exit(1)